...
const getData: QueryListHander<TenantDto> = async ({ queryForm, skipCount, maxResultCount, updateList }) => {
    const { totalCount, items } = await client.tenantsGET2(queryForm.filter, '', unref(skipCount), unref(maxResultCount))
